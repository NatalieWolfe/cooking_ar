...
Pose2d::Point json_to_point(const json& json_point) {
  if (!json_point.is_array() || json_point.size() != 4) {
    throw lw::InvalidArgument()
      << "Invalid point, expected array with 4 elements got: " << json_point;
  }
  return {
    .point_id = json_point[0].get<int>(),
    .x = json_point[1].get<double>(),
    .y = json_point[2].get<double>(),
    .confidence = json_point[3].get<double>()
  };
}

std::vector<Pose2d::Point> json_to_points(
...
Pose2d json_to_pose(const json& json_pose) {
