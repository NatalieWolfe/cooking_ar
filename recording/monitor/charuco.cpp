...
void CharucoCalibrator::set_latest_frame(cv::Mat frame, bool extract_board) {
  frame.copyTo(_frame);
  if (!extract_board) return;

  // _debug_text = _parameters.device.device_path.string() + '\n';
  _debug_text = "RMS: " + std::to_string(_error_rate) + '\n';

  auto charuco = extract_charuco(_frame);
  if (!charuco) return;
  std::tie(_last_corners, _last_corner_ids) = std::move(*charuco);
...
void CharucoCalibrator::save_latest_frame() {
  if (!_last_corners.empty()) {
    ...
    if (_saved_corners.size() >= MIN_CALIBRATION_FRAMES) calibrate();
  }
}
...
std::optional<std::pair<cv::Mat, cv::Mat>> CharucoCalibrator::extract_charuco(
  cv::Mat frame
) const {
  ...
  if (corner_ids.empty()) return std::nullopt;
  return std::make_pair(corners, corner_ids);
}
...
double CharucoCalibrator::calibrate() {
